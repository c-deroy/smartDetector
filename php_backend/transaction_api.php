...
function sendTransactionNotification(array $mailConfig, string $to, array $transaction, array $prediction)
{
    ...
    $message .= '<a href="https://localhost/smartDetector/app" style="display: inline-block; background-color: #1e40af; color: white; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600; font-size: 14px;">View in Dashboard</a>';
    $message .= '</div>';
    ...
    if (class_exists('\PHPMailer\PHPMailer\PHPMailer')) {
        try {
            ...
            if (!empty($smtp['enabled'])) {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = $smtp['username'] ?? 'apafloivan@gmail.com';
                $mail->Password = $smtp['password'] ?? 'macqcdcaqaaipmvm';
