...
const BACKEND_API = 'http://localhost:8000';  // Update this if backend runs on different port
...
function describeRiskSignal(feature, item) {
    const label = friendlyFeatureName(feature);
...
function buildPaySimFeaturePayload(tx) {
    const now = new Date(tx.timestamp || Date.now());
    const merchant = String(tx.merchant || tx.note || 'unknown').toLowerCase();
    const typeName = String(tx.type || tx.transaction_type || 'PAYMENT').toUpperCase();
    const amount = Math.max(0, Number(tx.amount) || 0);
    const merchantRisk = Number(tx.merchantRisk || 0.4);
    const isNight = now.getHours() < 6 || now.getHours() > 22;

    const featureMap = {
        step: Math.max(1, Math.round((Date.now() - new Date('2024-01-01').getTime()) / 3600000)),
        time_hour: now.getHours(),
        amount: amount,
        oldbalanceOrg: Math.max(100, amount * 2),
        newbalanceOrig: Math.max(0, amount * 1.4),
        oldbalanceDest: 0,
        newbalanceDest: 0,
        orig_amount_mean: Math.max(50, amount * 0.8),
        orig_amount_std: Math.max(0, amount * 0.2),
        time_since_prev: tx.time_since_prev || 5,
        tx_count_24h: tx.tx_count_24h || 1,
        amount_z: tx.amount_z || (amount > 0 ? Math.min(5, amount / 250) : 0),
        is_unusual_amount: tx.is_unusual_amount || (amount > 2000 ? 1 : 0),
        merchant_risk: merchantRisk,
        account_age_days: tx.account_age_days || 365,
        auth_failures: tx.auth_failures || 0,
        type_PAYMENT: typeName.includes('PAYMENT') || typeName.includes('POS') || typeName.includes('SHOP') ? 1 : 0,
        type_CASH_OUT: typeName.includes('CASH_OUT') || typeName.includes('WITHDRAW') ? 1 : 0,
        type_DEBIT: typeName.includes('DEBIT') ? 1 : 0,
        type_TRANSFER: typeName.includes('TRANSFER') || typeName.includes('WIRE') ? 1 : 0,
        merchant_cat_grocery: /grocery|market|food|shop/.test(merchant) ? 1 : 0,
        merchant_cat_restaurant: /restaurant|cafe|eat|food/.test(merchant) ? 1 : 0,
        merchant_cat_retail: /store|shop|mart|retail/.test(merchant) ? 1 : 0,
        merchant_cat_travel: /travel|flight|hotel|air|ticket/.test(merchant) ? 1 : 0,
        merchant_cat_utilities: /utility|telecom|data|water|energy|bill/.test(merchant) ? 1 : 0,
        country_CN: 0,
        country_DE: 0,
        country_FR: 0,
        country_GB: 0,
...
async function predictFraudBackend(tx) {
    try {
        const payload = buildPaySimFeaturePayload(tx);
        ...
        const status = riskScore >= loadSettings().alertThreshold ? 'Fraud' : 'Safe';

        return {
            riskScore,
            status,
            riskLevel: riskScore > 80 ? 'High Risk' : riskScore > 60 ? 'Medium Risk' : 'Low Risk',
    ...
    if (impacts && impacts.length) {
        return impacts.slice(0, 5).map(item => {
            ...
            const value = isGenericExplanation(tx)
                ...
                : Math.min(100, Math.max(5, Math.round(Math.abs(contribution) * 100)));
            return {
                label: signal.label,
                value
            };
